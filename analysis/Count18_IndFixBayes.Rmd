---
title: "Count 18 - Individual Fixed Effect"
author: "Yuxin Zou"
date: 2018-4-16
output: 
  html_document:
...
The type of bacteria (bact), and the time post-infection (time) are treated as fixed effects. The individual (ind) is treated as fixed effect as well.
...
design <- model.matrix(~ 0 + bact + ind + extr + rin, data = groups18)
...
```{r save}
saveRDS(list(beta = beta_class, sebeta = sebeta_class, t = t_class, df = df_class), '../output/Count18_IndFix_limmaBayes.rds')
```
