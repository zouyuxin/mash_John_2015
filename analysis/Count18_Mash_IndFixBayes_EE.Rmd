---
title: "Count 18 Mash - IndFixBayes - EE"
author: "Yuxin Zou"
date: 2018-4-16
output: 
  html_document:
...
saveRDS(cov_ed(mash.data.ed, U.dd), '../output/Count18_IndFixBayes_MashEE_Cov.rds')
```

## Mash model:
```{r, echo=FALSE}
U.ed = readRDS('../output/Count18_IndFixBayes_MashEE_Cov.rds')
```

```{r mash, eval=FALSE}
saveRDS(mash(mash.data.diff, c(U.c, U.ed), algorithm.version = 'R'), '../output/Count18_IndFixBayes_mash_model_EE.rds') 
```

# Result
```{r}
mash.model = readRDS('../output/Count18_IndFixBayes_mash_model_EE.rds')
```
...
for (j in 1:4)
  barplot(v[,j]/v[,j][which.max(abs(v[,j]))], cex.names = 0.7,
          las = 2, main = paste0("EigenVector ", j, " for Load_5"))
```
