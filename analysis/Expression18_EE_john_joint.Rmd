---
title: "Expression 18 Mash - EE - joint mean"
author: "Yuxin Zou"
date: 2020-12-9
output: 
  html_document:
...
saveRDS(fmodel, '../output/Expression18_EE_jointmean_flash.rds')
```

```{r cov from flash}
fmodel = readRDS('../output/Expression18_EE_jointmean_flash.rds')
...
saveRDS(cov_ed(data.strong.L, U.dd), '../output/Expression18_EE_jointmean_ED.rds')
```

```{r mash, eval=FALSE}
U.ed = readRDS('../output/Expression18_EE_jointmean_ED.rds')
saveRDS(mash(mash.data.L, c(U.c, U.ed)), '../output/Expression18_jointmean_mash_model.rds') 
```

```{r}
mash.model = readRDS('../output/Expression18_jointmean_mash_model.rds') 
```
...
data.strong = mash_set_data(Bhat = beta_class[idx,], Shat = sebeta_class[idx,])
data.strong.L = mash_update_data(data.strong, ref='none')
```
...
data.random = mash_set_data(Bhat = beta_class[idx,], Shat = sebeta_class[idx,])
data.random.L = mash_update_data(data.random, ref='none')
```
...
flash.data = flash_set_data(data.strong.L$Bhat)
fmodel = flash(flash.data, greedy = TRUE, backfit = TRUE)
saveRDS(fmodel, '../output/Expression18_EE_jointmean_subset_flash.rds')
```

```{r subset cov from flash}
fmodel = readRDS('../output/Expression18_EE_jointmean_subset_flash.rds')
Factors = fmodel$ldf$f
Flash_res = flash_get_fitted_values(fmodel)
U.Flash = c(mashr:::cov_from_factors(t(as.matrix(Factors)), "Flash"), 
            list("tFlash" = t(Flash_res) %*% Flash_res / nrow(Flash_res)))
...
X.center = apply(data.strong.L$Bhat, 2, function(x) x - mean(x))
U.dd = c(U.pca, U.Flash, list('XX' = t(X.center) %*% X.center / nrow(X.center)))
saveRDS(cov_ed(data.strong.L, U.dd), '../output/Expression18_EE_jointmean_subset_ED.rds')
```

```{r subset mash, eval=FALSE}
U.ed = readRDS('../output/Expression18_EE_jointmean_subset_ED.rds')
m.model = mash(data.random.L, c(U.c, U.ed), outputlevel = 1)
m.model$result = mash_compute_posterior_matrices(m.model, mash.data.L)
saveRDS(m.model, '../output/Expression18_jointmean_subset_mash_model.rds') 
```

```{r}
mash.model = readRDS('../output/Expression18_jointmean_subset_mash_model.rds') 
```
There are `r length(get_significant_results(mash.model))` DE genes.
