---
title: "Count 18 Mash - IndFixBayes - EET"
author: "Yuxin Zou"
date: 2018-4-16
output: 
  html_document:
...
```{r data}
data = readRDS('../output/Count_IndFix_limmaBayes.rds')
# Time 18
...
```{r flashLoading}
loading = fmodel$EL[,1:6]
colnames(loading) = paste0('F',seq(1,6))
...
saveRDS(cov_ed(mash.data.ed, U.dd), '../output/Count18_IndFixBayes_MashEET_Cov.rds')
```

## Mash model:
```{r, echo=FALSE}
U.ed = readRDS('../output/Count18_IndFixBayes_MashEET_Cov.rds')
```

```{r mash, eval=FALSE}
saveRDS(mash(mash.data.diff, c(U.c, U.ed), algorithm.version = 'R'), '../output/Count18_IndFixBayes_mash_model_EET.rds') 
```

# Result
```{r}
mash.model = readRDS('../output/Count18_IndFixBayes_mash_model_EET.rds')
```
...
```{r Load5}
x           <- cov2cor(mash.model$fitted_g$Ulist[["ED_Load_5"]])
x[x > 1]    <- 1
x[x < -1]   <- -1
colnames(x) <- colnames(beta_class)[2:9]
rownames(x) <- colnames(x)
corrplot.mixed(x, tl.pos="d",upper='color',cl.lim=c(0.99,1), upper.col=colorRampPalette(rev(c("#D73027","#FC8D59","#FEE090","#FFFFBF","#E0F3F8","#91BFDB","#4575B4")))(40), tl.cex=1.2)
```

```{r Load5 pca}
layout(matrix(c(1,2,3,4), 2, 2, byrow=TRUE))
svd.out = svd(mash.model$fitted_g$Ulist[["ED_Load_5"]])
v = svd.out$v
...
for (j in 1:4)
  barplot(v[,j]/v[,j][which.max(abs(v[,j]))], cex.names = 0.7,
          las = 2, main = paste0("EigenVector ", j, " for Load 5"))
```
