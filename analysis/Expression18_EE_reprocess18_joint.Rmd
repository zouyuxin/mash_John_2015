...
```{r data}
data = readRDS('../output/Expression_reprocess18_joint.rds')
...
saveRDS(cov_ed(data.strong.L, U.dd), '../output/Expression18_reprocess18_EE_jointmean_ED.rds')
```

## Mash model

```{r}
U.ed = readRDS('../output/Expression18_reprocess18_EE_jointmean_ED.rds')
```

```{r mash, eval=FALSE}
U.dd.diag = c(U.ed[c('ED_tPCA', 'ED_tFlash', 'ED_XX')], lapply(U.ed[c('ED_PCA_1','ED_PCA_2','ED_PCA_3', paste0('ED_Flash_',1:5))], function(U) U + 0.01*diag(8)))
mash.model = mash(mash.data.L, c(U.c, U.dd.diag))
saveRDS(mash.model, '../output/Expression18_reprocess18_EE_jointmean_mash_model.rds') 
```

## Result

```{r}
mash.model = readRDS('../output/Expression18_reprocess18_EE_jointmean_mash_model.rds')
```
