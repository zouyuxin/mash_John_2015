---
title: "Count 18 Mash - IndRanBayes - EE - Wrong"
author: "Yuxin Zou"
date: 2018-5-9
output: 
  html_document:
...
L[,1] = -1
row.names(L) = colnames(beta_class)
L = L[2:9,]
Bhat = mash.data$Bhat %*% t(L)
Shat = sqrt(mash.data$Shat[,2:9]^2 + mash.data$Shat[,1]^2)
mash.data.diff = mash_set_data(Bhat = Bhat, Shat = Shat)
```
...
saveRDS(cov_ed(mash.data.ed, U.dd), '../output/Count18_IndRanBayes_MashEE_Cov_Wrong.rds')
```

## Mash model:
```{r}
U.ed = readRDS('../output/Count18_IndRanBayes_MashEE_Cov_Wrong.rds')
```

```{r mash, eval=FALSE}
saveRDS(mash(mash.data.diff, c(U.c, U.ed)), '../output/Count18_IndRanBayes_mash_model_EE_Wrong.rds') 
```

# Result
```{r}
mash.model = readRDS('../output/Count18_IndRanBayes_mash_model_EE_Wrong.rds')
```
