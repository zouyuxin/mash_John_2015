---
title: "Count 18 - Individual Random Effect"
author: "Yuxin Zou"
date: 2018-4-16
output: 
  html_document:
...
```{r model}
contrasts(groups18$extr) = contr.sum(levels(groups18$extr))
design <- model.matrix(~ 0 + bact + extr + rin, data = groups18)
...
v = voom(y, design, block = groups18$ind, correlation = corfit$consensus.correlation)
```
...
```{r save}
saveRDS(list(beta = beta_class, sebeta = sebeta_class, t = t_class, df = df_class), '../output/Count18_IndRan_limmaBayes.rds')
```
