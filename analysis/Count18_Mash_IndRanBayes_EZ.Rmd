---
title: "Count 18 Mash - IndRanBayes - EZ"
author: "Yuxin Zou"
date: 2018-4-16
output: 
  html_document:
...
```{r data}
data = readRDS('../output/Count18_IndRan_limmaBayes.rds')
...
saveRDS(cov_ed(mash.data.ed, U.dd), '../output/Count18_IndRanBayes_MashEZ_Cov.rds')
```

## Mash model:
```{r, echo=FALSE}
U.ed = readRDS('../output/Count18_IndRanBayes_MashEZ_Cov.rds')
```

```{r mash, eval=FALSE}
saveRDS(mash(mash.data.diff, c(U.c, U.ed), algorithm.version = 'R'), '../output/Count18_IndRanBayes_mash_model_EZ.rds') 
```

# Result
```{r}
mash.model = readRDS('../output/Count18_IndRanBayes_mash_model_EZ.rds')
```
...
```{r Load_3}
x           <- cov2cor(mash.model$fitted_g$Ulist[["ED_Load_3"]])
x[x > 1]    <- 1
x[x < -1]   <- -1
...
svd.out = svd(mash.model$fitted_g$Ulist[["ED_Load_3"]])
v = svd.out$v
...
for (j in 1:4)
  barplot(v[,j]/v[,j][which.max(abs(v[,j]))], cex.names = 0.7,
          las = 2, main = paste0("EigenVector ", j, " for Load_3"))
```
