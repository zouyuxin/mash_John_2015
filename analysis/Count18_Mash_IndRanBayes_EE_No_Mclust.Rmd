...
png('~/Documents/GitHub/MASH_baseline/figures/John18_flash_Factors.png', width = 700, height = 500)
par(mfrow=c(2,2))
for(i in pve.order){
...
saveRDS(cov_ed(mash.data.diff, U.dd, subset = strong), '../output/Count18_IndRanBayes_MashEE_Cov_NOMclust.rds')
```

## Mash model:
```{r}
U.ed = readRDS('../output/Count18_IndRanBayes_MashEE_Cov_NOMclust.rds')
```

```{r mash, eval=FALSE}
saveRDS(mash(mash.data.diff, c(U.c, U.ed), algorithm.version = 'R'), '../output/Count18_IndRanBayes_mash_model_EE_NOMclust.rds') 
```

# Result
```{r}
mash.model = readRDS('../output/Count18_IndRanBayes_mash_model_EE_NOMclust.rds')
```
...
Check Flash 1 covariance matrix

```{r Load6}
x           <- mash.model$fitted_g$Ulist[["ED_Flash_1"]]
colnames(x) <- colnames(mash.data.diff$Bhat)
rownames(x) <- colnames(x)
png('~/Documents/GitHub/MASH_baseline/figures/John18CovFactor1.png', width = 500, height=500)
...
svd.out = svd(mash.model$fitted_g$Ulist[["ED_Flash_1"]])
v = svd.out$v
...
for (j in 1:4)
  barplot(v[,j]/v[,j][which.max(abs(v[,j]))], cex.names = 0.7,
          las = 2, main = paste0("EigenVector ", j, " for ED_Flash_1"))
```
-->
...
colnames(x) <- colnames(beta_class)[-1]
rownames(x) <- colnames(x)
png(filename = '~/Documents/GitHub/MASH_baseline/figures/John18PS_Sign.png', width=500, height = 500)
corrplot.mixed(x, tl.pos="d",upper='color', cl.lim=c(0,1), upper.col=colorRampPalette(rev(c("#D73027","#FC8D59","#FEE090","#FFFFBF",
                               "#E0F3F8","#91BFDB","#4575B4")))(40),
...
saveRDS(mash.model.sample, '../output/Count18_IndRanBayes_mash_model_EE_NOMclust_samples.rds')
```

```{r}
mash.model.sample = readRDS('../output/Count18_IndRanBayes_mash_model_EE_NOMclust_samples.rds')
```
...
colnames(x) <- colnames(beta_class)[-1]
rownames(x) <- colnames(x)
png(filename = '~/Documents/GitHub/MASH_baseline/figures/John18PS_Sample_Sig_Sign.png', width=500, height = 500)
...
colnames(x) <- colnames(beta_class)[-1]
rownames(x) <- colnames(x)
png(filename = '~/Documents/GitHub/MASH_baseline/figures/John18PS_Sample_Sig_Mag.png', width=500, height = 500)
