---
title: "Count - Individual Random Effect - Bayes"
author: "Yuxin Zou"
date: 2018-4-16
output: 
  html_document:
...
batch <- model.matrix(~ extr + rin, data = groups)
...
```{r voom}
v = voom(y, design)
corfit = duplicateCorrelation(v, design, block = groups$ind)
v = voom(y, design, block = groups$ind, correlation = corfit$consensus.correlation)
```

* limma
```{r limmaRandomEffect}
fit = lmFit(v, design, block = groups$ind, correlation = corfit$consensus.correlation)
fitB = eBayes(fit)
```
...
```{r save}
saveRDS(list(beta = beta_class, sebeta = sebeta_class, t = t_class, df = df_class, groups = groups), '../output/Count_IndRan_limmaBayes.rds')
```
