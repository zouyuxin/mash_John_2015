...
```{r}
factors = flash_get_ldf(flash.model.l)$f
row.names(factors) = paste0('F', seq(1:5))
pve.order = order(flash_get_pve(flash.model.l), decreasing = TRUE)

par(mfrow=c(2,2))
for(i in pve.order){
  barplot(factors[,i], main=paste0('Factor ',i, ' pve= ', round(flash_get_pve(flash.model.l)[i],3)), las=2, cex.names = 0.7)
}
par(mfrow=c(1,1))
```
...
```{r}
factors = flash_get_ldf(flash.model.lc)$f
row.names(factors) = paste0('F', seq(1:5))
pve.order = order(flash_get_pve(flash.model.lc), decreasing = TRUE)

par(mfrow=c(1,2))
for(i in pve.order){
  barplot(factors[,i], main=paste0('Factor ',i, ' pve= ', round(flash_get_pve(flash.model.lc)[i],3)), las=2, cex.names = 0.7)
}
par(mfrow=c(1,1))
```
