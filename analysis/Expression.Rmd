...
beta_class <- matrix(0, nrow(dat_cpm), ncol(design));
sebeta_class <- matrix(0, nrow(dat_cpm), ncol(design))
t_class = matrix(0, nrow(dat_cpm), ncol(design))
df_class = matrix(0, nrow(dat_cpm), ncol(design))
...
for(k in c(1:9)){
  ...
  fit<-eBayes(l)
  beta_class[,k] <- fit$coefficients;
  sebeta_class[,k] <- fit$stdev.unscaled * sqrt(fit$s2.post)
  t_class[,k] = fit$t
  df_class[,k] = fit$df.total
}
```
